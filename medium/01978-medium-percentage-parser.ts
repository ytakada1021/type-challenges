...
type PercentExtractor<S extends string> = S extends `${infer T}%` ? [T, "%"] : [S, ""];

type PercentageParser<A extends string> = A extends `+${infer T}`
  ? ["+", ...PercentExtractor<T>]
  : A extends `-${infer U}`
  ? ["-", ...PercentExtractor<U>]
  : ["", ...PercentExtractor<A>];

type Foo = "s" extends `${never}s` ? true : false;
