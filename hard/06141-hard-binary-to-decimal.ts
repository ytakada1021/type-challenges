...
type BinaryToDecimalByTuple<T extends Bit[], CurrentDigit extends 0[] = [0]> = T extends [
  ...infer Rest extends Bit[],
  infer Last
]
  ? Last extends "1"
    ? [...CurrentDigit, ...BinaryToDecimalByTuple<Rest, [...CurrentDigit, ...CurrentDigit]>]
    : BinaryToDecimalByTuple<Rest, [...CurrentDigit, ...CurrentDigit]>
  : [];

type BinaryToDecimal<S extends string> = BinaryToDecimalByTuple<StringToTuple<S>>["length"];
