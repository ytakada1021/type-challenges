...
type CamelCaseLower<S extends string> = S extends `${infer First}${infer Rest}`
  ? First extends "_"
    ? Rest extends `${infer L}${string}`
      ? Uppercase<L> extends Lowercase<L>
        ? `${First}${CamelCaseLower<Rest>}`
        : `${CamelCaseLower<Capitalize<Rest>>}`
      : "_"
    : `${First}${CamelCaseLower<Rest>}`
  : "";
