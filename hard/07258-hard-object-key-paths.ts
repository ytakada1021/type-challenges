...
type ObjectKeyPaths<T, IsHead = true, U extends keyof T = keyof T> = T extends unknown[]
  ? U extends number
    ?
        | `${IsHead extends true ? "" : "."}${
            | U
            | `[${U}]`
            | `${U}${ObjectKeyPaths<T[U], false>}`
            | `[${U}]${ObjectKeyPaths<T[U], false>}`}`
        | `[${U}]`
        | `[${U}]${ObjectKeyPaths<T[U], false>}`
    : never
  ...
  ? U extends string | number
    ? `${IsHead extends true ? "" : "."}${U | `${U}${ObjectKeyPaths<T[U], false>}`}`
    : never
  : never;

/* _____________ Test Cases _____________ */
import type { Equal, Expect, ExpectExtends } from "../util-types";
