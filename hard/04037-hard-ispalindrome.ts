...
type IsTuplePalindrome<T extends unknown[]> = T extends [infer First, ...infer Rest, infer Last]
  ? First extends Last
    ? IsTuplePalindrome<Rest>
    : false
  : true;

type IsPalindrome<T extends string | number> = IsTuplePalindrome<StringToTuple<`${T}`>>;
